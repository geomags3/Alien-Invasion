...
def change_fleet_directions(ai_settings, aliens):
    ...
    ai_settings.fleet_direction *= -1  # Меняем направление флота
...
def update_aliens(ai_settings, stats, screen, ship, aliens, bullets):
    ...
    aliens.update()  # Вызывает метод update() для каждого пришельца в группе aliens

    # Проверка коллизий "пришелец-корабль"
    if pygame.sprite.spritecollideany(ship, aliens):
        ship_hit(ai_settings, stats, screen, ship, aliens, bullets)

    # Проверка пришельцев, добравшихся до нижного края экрана
...
def check_bullet_alien_collisions(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    # Удаление пуль и пришелььцев, участвующих в коллизиях
    ...
    if len(aliens) == 0:
        # Уничетожить существующие пули и создать новый флот
        bullets.empty()
...
def check_events(ai_settings, screen, stats, play_button, ship, aliens, bullets):
    ...
    for event in pygame.event.get():  # Цикл обработки событий
        if event.type == pygame.QUIT:  # Выход из игры по событию
            sys.exit()
